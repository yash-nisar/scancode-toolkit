...
import json2csv_legacy


class TestJson2CSVLegacy(FileBasedTesting):
    test_data_dir = os.path.join(os.path.dirname(__file__), 'testdata')

    def test_scanc_as_list_minimal(self):
        test_json = self.get_test_loc('json2csv_legacy/minimal.json')
        scan = json2csv_legacy.load_scan(test_json)
        result = json2csv_legacy.scan_as_list(scan)
        expected = self.get_test_loc('json2csv_legacy/minimal.json-expected')
    ...
    def test_scanc_as_list_minimal_with_strip(self):
        test_json = self.get_test_loc('json2csv_legacy/minimal.json')
        scan = json2csv_legacy.load_scan(test_json)
        result = json2csv_legacy.scan_as_list(scan, strip=2)
        expected = self.get_test_loc('json2csv_legacy/minimal.json-strip-expected')
        expected = json.load(open(expected), object_pairs_hook=OrderedDict)
        assert expected == result

    def test_scanc_as_list_full(self):
        test_json = self.get_test_loc('json2csv_legacy/full.json')
        scan = json2csv_legacy.load_scan(test_json)
        result = json2csv_legacy.scan_as_list(scan)
        expected = self.get_test_loc('json2csv_legacy/full.json-expected')
        expected = json.load(open(expected), object_pairs_hook=OrderedDict)
        assert expected == result

    def test_json2csv_legacy_minimal(self):
        test_json = self.get_test_loc('json2csv_legacy/minimal.json')
        result_file = self.get_temp_file('.csv')
        json2csv_legacy.json_scan_to_csv(test_json, result_file)
        expected_file = self.get_test_loc('json2csv_legacy/minimal.csv')
        expected = codecs.open(expected_file, 'rb', encoding='utf-8').read()
        result = codecs.open(result_file, 'rb', encoding='utf-8').read()
        assert expected == result

    def test_key_ordering(self):
        test_json = self.get_test_loc('json2csv_legacy/key_order.json')
        scan = json2csv_legacy.load_scan(test_json)
        result = json2csv_legacy.scan_as_list(scan)
    ...
    def test_json_with_no_keys_does_not_error_out(self):
        ...
        test_json = self.get_test_loc('json2csv_legacy/no_keys.json')
        scan = json2csv_legacy.load_scan(test_json)
        result = json2csv_legacy.scan_as_list(scan)
        assert [] == result

    def test_can_process_html_app_and_regular_json_the_same_way(self):
        test_html = self.get_test_loc('json2csv_legacy/format_html_app_data.json')
        test_json = self.get_test_loc('json2csv_legacy/format_json_scan.json')
        assert json2csv_legacy.load_scan(test_html) == json2csv_legacy.load_scan(test_json)
